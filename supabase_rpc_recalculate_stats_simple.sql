-- Simplified Supabase RPC Function to Recalculate User Stats
-- This version only uses columns that exist in the user_stats table
...
BEGIN
    ...
    -- Using only columns that exist: user_id, matches_played, wins, losses, draws, total_goals, total_assists, leagues_created
    ...
    SELECT 
        ...
        COALESCE(stats.goals_conceded, 0) as total_assists,  -- Store goals conceded in assists field
...
-- Comment
COMMENT ON FUNCTION recalculate_all_user_stats() IS 'Recalculates user statistics from all completed matches. Bypasses RLS policies. Uses only existing columns.';
